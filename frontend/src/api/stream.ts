...
/** fetch 流同样需要登录态，未显式指定的头由 tokenStore 补齐 */
function withAuthHeaders(init: RequestInit): RequestInit {
  const headers = new Headers(init.headers);
  for (const [name, value] of Object.entries(getAuthHeader())) {
    if (!headers.has(name)) {
...
export async function streamSse(options: StreamSseOptions): Promise<void> {
  try {
    const response = await fetch(toApiUrl(options.url), withAuthHeaders(options.init));
